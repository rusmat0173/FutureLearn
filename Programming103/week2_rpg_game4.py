...
"""
# import json library for saving player details if they quit
import json
...
class Player:
    def __init__(self):
        ...
        try:
            ...
            with open("gamedata.json", "r") as f:
                ...
                self.name = gamedata["playername"]
                self.health = gamedata["playerhealth"]
                self.player_position = gamedata["playercurrentRoom"]
                self.inventory = gamedata["playerLoot"]
        except:
    ...
    def new_direction(self, input): # <= added input
        # self.nsew = input()
        self.nsew = input
        if self.nsew == 'North':
            self.delta = [-1,0]
    ...
    def quit(self):
        gamedata = {
            ...
            "playercurrentRoom": self.player_position,
            "playerLoot": self.inventory
        }
    ...
    def room_actions(self):
        ...
        if room_loot == "Monster":
            ...
            if X == "Yes":
                ...
                print(self.player.inventory, self.player.player_position)
...
while game_go:
    ...
    while check_function == 0:
        print("Which Direction or Quit?")
        response = input()
        if response in ["North", "South", "East", "West"]:
            playerOne.new_direction(response)
            ...
            playerOne.quit()
            print("Thank you for taking part. See you next time.")
            game_go = False # <= breaks out of the while True loop
            check_function = 1 # <= breaks out of the while check_function == 0 loop
